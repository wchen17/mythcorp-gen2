...
export type UploadStatus =
  ...
  | { kind: 'done'; url: string; viewUrl: string; deleteUrl: string; type: string; size: number }
...
export function useUpload(apiKey: string) {
  ...
  const upload = useCallback((file: File) => {
    ...
    request.onload = () => {
      let data: { url?: string; viewUrl?: string; deleteUrl?: string; error?: string } = {}
      try { data = JSON.parse(request.responseText) as typeof data; } catch { data = {}; }
      if (request.status < 200 || request.status >= 300 || !data.url || !data.viewUrl || !data.deleteUrl) { setStatus({ kind: 'error', message: data.error ?? `Upload failed (${request.status}).` }); return; }
      setStatus({ kind: 'done', url: data.url, viewUrl: data.viewUrl, deleteUrl: data.deleteUrl, type: file.type, size: file.size });
    };
